			...
			<table cellpadding="0" cellspacing="0" border="0" width="100%" height="100%">
				<tr valign="top">
					<td>
						...
						<div class="socnet-chat-info" id="socnet_chat_info" style="display: none">
						...
						<div class="socnet-chat-form" id="socnet_chat_form">
							<div class="socnet-chat-buttons" style="display: none">
							...
							<div class="smiles-div" id="smiles">
								<?
								$res_str = '';
								$size = "40px";
								foreach($arResult["PrintSmilesList"] as $res)
								{
									...
									if (IntVal($res['IMAGE_WIDTH'])>0) {$res_str .= " width=\"".$size."\"";}
									if (IntVal($res['IMAGE_HEIGHT'])>0) {$res_str .= " height=\"".$size."\"";}
							...
							<div class="socnet-chat-input">
								<div class="group-border">
									<img src="/resources/images/icons/smile.jpg" class="smile-button" onclick="showSmile()">
									<?=bitrix_sessid_post()?>
									<input type="text" id="post_message_id" name="POST_MESSAGE" rows="1" class="input-line">
									<input id="post_message_button" type="submit" name="save" value="SEND" class="send-button"><br>
								</div>
							</div>
						</div>
					...
					//contact list
					?>
					<td class="socnet-user-group-divider" valign="middle" onmousedown="sonet_start_drag(arguments[0]);" style="display: none">
...
<?
//Выводим адрес фото собоседника для передачи в клиент приложения
$id = $arResult["User"]["ID"];
$path = CFile::GetPath($arResult["User"]["PersonalPhotoFile"]["SRC"]);
$value = "http://student-app.ru".$path;
$APPLICATION->SetPageProperty("title", $value);
?>
<script>
	function showSmile(){
		var isHidden = document.getElementById('smiles').style.display;
		if(isHidden == "none"){
