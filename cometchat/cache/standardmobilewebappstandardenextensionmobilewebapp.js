            ...
            if(bannedOrKicked[1]=='kicked'||bannedOrKicked[1]=='banned'||bannedOrKicked[1]=='deletemessage'){
                ...
                if($("#cometchat_message_"+incoming.id).length>0){
                    ...
                    if(incoming.fromid!=jqcc.cometchat.getChatroomVars('myid')){
                        ...
                        temp += ('<div class="cometchat_chatboxmessage me" id="'+incoming.id+'"><div style="float:left; width:40px; height:40px; float:left; background: url(/upload/main/c59/c59d24a7fa4194ca1d8f2f9cf66d1444.jpg) no-repeat center; background-size: cover"></div><div style="margin-left:50px;"></table><span class="cometchat_chatboxmessagefrom"><strong>Я:</strong></span></br><span class="cometchat_chatboxmessagecontent" id="'+incoming.id+'">'+incoming.message+'</span><span class="cometchat_ts" id="'+incoming.id+'"></span></div><div style="clear:both;"></div></div><div class="userSeperator"></div>');
                    }
                }
            }
        });
    ...
    updateChatroomUsers: function(item, fetchedUsers){
        ...
        $.each(item, function(i, user){
            ...
            if(jqcc.cometchat.getcrAllVariables().users[user.id]!=1&&jqcc.cometchat.getChatroomVars('initializeRoom')==0&&jqcc.cometchat.getChatroomVars('hideEnterExit')==0){
                var ts = new Date();
                $("#currentroom_convotext").append('<div class="cometchat_chatboxalert" id="cometchat_message_0">'+user.n+' присоединился к беседе'+jqcc.mobilewebapp.getTimeDisplay(ts)+'</div><div style="clear:both;"></div>');
                setTimeout(function(){
                    jqcc.mobilewebapp.crscrollToBottom();
                }, 500);
            }
        ...
        for(user in jqcc.cometchat.getChatroomVars('users')){
            if(jqcc.cometchat.getChatroomVars('users').hasOwnProperty(user)){
                if(newUsers[user]!=1&&jqcc.cometchat.getChatroomVars('initializeRoom')==0&&jqcc.cometchat.getChatroomVars('hideEnterExit')==0){
                    var ts = new Date();
                    $("#currentroom_convotext").append('<div class="cometchat_chatboxalert" id="cometchat_message_0">'+jqcc.cometchat.getcrAllVariables().usersName[user]+' покинул беседу'+jqcc.mobilewebapp.getTimeDisplay(ts)+'</div><div style="clear:both;"></div>');
                    setTimeout(function(){
                        jqcc.mobilewebapp.crscrollToBottom();
                    }, 500);
                }
        ...
        if(bannedOrKicked[1]=='kicked'||bannedOrKicked[1]=='banned'){
            ...
            if($("#cometchat_message_"+incomingid).length>0){
                ...
                if(fromid!=jqcc.cometchat.getChatroomVars('myid')){
                    ...
                    $("#currentroom_convotext").append('<div class="cometchat_chatboxmessage me" id="cometchat_message_'+incoming.id+'"><div style="float:left">/upload/main/c59/c59d24a7fa4194ca1d8f2f9cf66d1444.jpg</div><div style="margin-left:50px;"></table><span class="cometchat_chatboxmessagefrom"><strong>'+fromname+':</strong></span></br><span class="cometchat_chatboxmessagecontent" id="'+incoming.id+'">'+incoming.message+'</span>'+jqcc.mobilewebapp.getLikeDisplay(id)+'</div><div style="clear:both;"></div></div><div class="userSeperator"></div>');
                }
